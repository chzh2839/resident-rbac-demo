...
Refresh Token / 블랙리스트가 Redis에 저장되므로, 테스트 실행 전 `docker-compose up -d`로 Redis를 먼저 띄워야 한다(12절 참고). Redis가 떠 있지 않으면 `@SpringBootTest`가 애플리케이션 컨텍스트를 올리는 단계에서부터 실패한다.
